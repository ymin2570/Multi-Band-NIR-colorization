...
from networks_dense import UNetKD
...
# Training settings
parser = argparse.ArgumentParser(description='color-teacher for colorization')
...
net = UNetKD(in_ch=6, out_ch=3, bilinear=False).cuda()
...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    for iteration, (data, target) in enumerate(dataloader):
        ...
        """ NETWORK """
        optimizer.zero_grad()
        _, _, _, _, _, _, _, _, _, out = net(torch.cat([data, target], dim=1))
